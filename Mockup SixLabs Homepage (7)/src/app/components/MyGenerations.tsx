  ...
  return (
    <div className="flex min-h-screen bg-surface font-body text-body-text overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden pt-[60px] lg:pt-0 lg:p-2 xl:p-3 2xl:p-4">
        ...
        {/* Header */}
        <header className="bg-white border-b border-border-subtle px-4 sm:px-6 md:px-8 py-4 sm:py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 z-20 shadow-sm shrink-0">
          <div className="flex items-center gap-3 sm:gap-6">
            <SectionHeading as="h1" className="text-2xl sm:text-3xl tracking-tight">Library</SectionHeading>
            <Badge variant="indigo">{generations.length} items</Badge>
          </div>

          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none sm:w-56 md:w-72">
              <SearchLg className="absolute left-3 sm:left-3.5 top-1/2 -translate-y-1/2 text-muted-text size-4 sm:size-5" />
              <Input
                placeholder="Search campaigns..."
                className="pl-10 sm:pl-11 h-10 sm:h-11 rounded-xl bg-surface/50 border-border-subtle focus:bg-white transition-all shadow-inner text-sm sm:text-base"
              />
            </div>
            <div className="flex items-center gap-2 sm:gap-4">
              <Button variant="outline" className="flex-1 sm:flex-none h-10 sm:h-11 px-4 sm:px-5 rounded-xl border-border-subtle gap-2 font-bold text-[13px] sm:text-[14px] shadow-sm hover:bg-surface transition-all">
                <FilterLines className="size-4 sm:size-5" /> Filter
              </Button>
              <Link to="/app/create" className="flex-1 sm:flex-none">
                <Button className="w-full bg-signal text-white h-10 sm:h-11 px-4 sm:px-6 rounded-xl gap-2 font-bold text-[13px] sm:text-[14px] hover:bg-signal/90 shadow-cta">
                  New <Zap className="size-4 sm:size-5" />
                </Button>
              </Link>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 lg:space-y-10">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
               <div className="space-y-1">
                 <h2 className="text-lg sm:text-xl font-display font-bold text-ink tracking-tight">Creative Generations</h2>
                 <p className="text-[13px] sm:text-sm text-muted-text">View and manage all your historical creative output.</p>
               </div>
               <BadgeGroup badge="Auto-Sync" className="text-xs sm:text-sm">
