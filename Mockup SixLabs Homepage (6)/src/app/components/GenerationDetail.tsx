  ...
  return (
    <div className="flex min-h-screen bg-surface font-body text-body-text overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden pt-[60px] lg:pt-0 lg:p-2 xl:p-3 2xl:p-4">
        <div className="flex-1 bg-white lg:rounded-[20px] xl:rounded-[24px] lg:border border-[#F2F0EB] lg:shadow-sm lg:hover:shadow-md lg:hover:border-[#D0D5DD] transition-all duration-300 flex flex-col overflow-hidden relative">
        {/* Header */}
        <header className="bg-white border-b border-border-subtle px-4 sm:px-6 md:px-8 py-3 sm:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 z-20 shrink-0">
          <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
            <Link to="/app" className="p-1.5 sm:p-2 hover:bg-surface rounded-full transition-colors text-muted-text hover:text-ink shrink-0">
              <ChevronLeft size={18} className="sm:w-5 sm:h-5" />
            </Link>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-lg sm:text-xl font-display font-bold text-ink truncate">Summer Collection 2026</h1>
                <Badge variant="indigo">Ready for review</Badge>
              </div>
              <p className="text-[11px] sm:text-[12px] text-muted-text">Generated 2h ago • Cirkul Workspace</p>
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-none rounded-xl border-border-subtle hover:bg-surface gap-2 font-bold text-[12px] sm:text-[13px] h-9 sm:h-10">
            ...
            <CTAButton className="flex-1 sm:flex-none px-4 sm:px-6 py-2 sm:py-2.5 h-9 sm:h-auto text-[12px] sm:text-[14px]">
              <CircleCheckBig size={14} className="sm:w-4 sm:h-4" /> <span className="hidden sm:inline">Approve</span>
            </CTAButton>
          </div>
        </header>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Main Preview Pane */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-surface/50">
            <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
              {/* Main Ad Preview - Inspired by image-2.png */}
              <Card className="rounded-xl sm:rounded-2xl border-border-subtle shadow-card bg-white overflow-hidden">
                <div className="flex flex-col lg:flex-row">
                  <div className="w-full lg:w-[450px] aspect-[4/5] bg-ink/5 relative group">
                    <ImageWithFallback 
                      src={uiImg3} 
                      alt="Ad Preview" 
                  ...
                  <div className="flex-1 p-5 sm:p-6 md:p-8 space-y-6 sm:space-y-8">
                    <div className="space-y-4 sm:space-y-6">
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:gap-4">
                        <Card className="rounded-lg sm:rounded-xl border-border-subtle bg-surface p-3 sm:p-4">
              ...
              <section className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-[13px] sm:text-[14px] font-bold text-ink">Batch Variations (6)</h4>
                  <Button variant="link" className="text-signal p-0 h-auto font-bold text-[12px] sm:text-[13px]">View Grid</Button>
                </div>
          ...
          <aside className="hidden xl:block w-96 bg-white border-l border-border-subtle overflow-y-auto">
